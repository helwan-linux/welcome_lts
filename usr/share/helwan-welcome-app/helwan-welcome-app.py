...
class WelcomeApp(QWidget):
	...
	def install_linux_ml(self):
		...
		cmd_logic = (
			...
			"echo \"Installation successful. Press 'Y' to set linux kernel as default.\" && "
			"read -r -p \"Set linux as default kernel? (y/N): \" response && "
			"if [[ \"$response\" =~ ^([yY][eE][sS]|[yY])$ ]]; then "
			"grub-set-default \"Advanced options for Arch Linux>Arch Linux, with Linux\" && "
			"echo \"Linux kernel set as default.\" ; "
			"else "
			"echo \"Linux kernel installed but not set as default.\" ; "
			"fi ; "
